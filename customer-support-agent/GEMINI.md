...
If you encounter `Error: Local server did not start within 30s` when running commands (e.g., `agents-cli run` or `agents-cli playground`):

1. **Synchronize Dependencies**: Run `agents-cli install` first to sync the local `.venv`.
2. **Manual Server Startup**: Manually run the FastAPI app server in a separate shell or process:
   ```bash
   uv run python -m uvicorn app.fast_api_app:app --host 127.0.0.1 --port 8000
   ```
3. **Query via Remote URL**: Query the manually started server by specifying the `--url` and `--mode` options:
   ```bash
   agents-cli run --url http://127.0.0.1:8000 --mode adk "your query"
   ```
