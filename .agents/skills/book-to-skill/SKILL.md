---
name: book-to-skill
description: "Converts books and documents (PDF, EPUB, DOCX, HTML, Markdown, plain text, RTF, MOBI/AZW with Calibre) into structured agent skills, extracting frameworks, mental models, principles, techniques, and anti-patterns. Trigger on /book-to-skill, book-to-skill, convert book, 'turn this book into a skill', or when converting books, guides, or PDFs into skills."
---
...
Books and technical whitepapers contain crystallized expertise: frameworks, architectures, principles, and engineering techniques that took years to develop. This skill extracts that knowledge into a format agents can leverage repeatedly.

**Extract structure and physics, not superficial summaries.** A skill isn't a book report. It's a practitioner's toolkit of:
- Named frameworks & architectures (mental models and system block diagrams with clear application)
- Physical & technical constraints (exact voltage rails, clock frequencies, bus widths, memory footprints, and node geometries)
- Deep engineering trade-off questions (mixed-signal isolation, bus contention, thermal dissipation, clock jitter, failure modes)
- Actionable principles (rules that guide architectural and operational decisions)
- Standard compliance mapping (AEC-Q100, ISO 26262 ASIL-D, MIL-STD-883K, DO-160G, PIL / Make-II)
- Anti-patterns & failure modes (what breaks and why)

**Strict Vector Asset & Visual Cockpit Protocol (Zero Hallucination / Zero Ugly Re-Draws):**
- **NEVER** use AI image generation (`generate_image`) to fabricate fictional schematics, circuit pinouts, or architecture sheets.
- **NEVER** redraw crude, simplified HTML/SVG approximations that strip away authentic engineering fidelity.
- **MANDATE:** Extract 100% authentic 1:1 vector PostScript/PDF pages (or 300+ DPI PyMuPDF extractions) into `assets/figures/` and preserve the author's original waveforms, pinouts, and block diagrams.
- **MANDATE The Silicon & Engineering Cockpit Standard:**
  - **Do NOT just dump raw white PDF pages into a blank dark canvas.** A raw 8.5×11 white PDF page causes blinding contrast, hides diagrams behind 2 inches of white margin, and exhibits text collisions from PDF running headers.
  - **Schematic-First Focus Mode:** Provide 300+ DPI isolated figure crops (`assets/figures/figXX_...png`) centered on an illuminated semiconductor blueprint stage with pan/zoom controls.
  - **Dual Theme Lighting:** Provide an interactive **Dark HUD Mode** (high-contrast inverted circuit tracing where copper/traces glow cyan/gold, matching EDA tool suites like Cadence/Virtuoso) alongside a **Clean Studio Sheet Mode**.
  - **Rich Visual Navigation:** Navigation sidebars must feature visual thumbnail previews, process node chips (`130nm BCD`, `77GHz BiCMOS`), and live search filters—never flat plain text lists.
  - **Multi-Tab Socratic HUD:** Accompany every figure with an interactive multi-tab inspector covering:
    1. *Silicon Specs*: Physical process nodes, rails, clock frequencies, and standards.
    2. *Socratic Red-Team Q&A*: Hard circuit physics trade-offs, substrate noise, thermal runaway, and failure modes.
    3. *Unit Economics & Moat*: Target ASP ($), foundry wafer costs, package/screening overhead, gross margins, and PIL domestic value addition (DVA > 65%).
    4. *Registers & Drivers*: Memory-mapped register definitions, bitfields, and C/Rust driver snippets.
  - **Multi-Mode Console:** Include toggles for **Schematic Focus (300 DPI)**, **Full PDF Page (PDF.js)**, **Figure Gallery (25-sheet card wall)**, and **Interactive Economics Simulator**.

**Preserve the author's precision.** Frameworks and pinouts often have specific names for reasons. "The 5 Whys" isn't interchangeable with "ask why multiple times." Capture the exact formulation.

**Layer depth appropriately.** Simple books → simple skills. Complex technical whitepapers with 10+ subsystem SKUs → deep modular skills with reference files, chapter audits, and native vector consoles.
...
## Core Idea / Thesis
<1–2 sentences: the single most important architectural or conceptual truth this chapter teaches>

## Authentic Vector Reference *(Technical books/whitepapers)*
- **Figure Reference**: `Fig <N>: <Caption>` (Page <P> in source PDF)
- **Extracted Asset**: `assets/figures/fig<NN>_<slug>.png`

## Physical & Electrical Specifications *(Technical books/whitepapers)*
| Parameter | Specification / Value |
|---|---|
| **Process Node / Geometry** | <e.g., 130nm BCD / 180nm CMOS / 28nm FD-SOI> |
| **Voltage Rails & Tolerances** | <e.g., 5V - 120V HV rail, 1.8V core, 3.3V I/O> |
| **Clock Frequencies & Domains** | <e.g., 200 MHz Core, 50 MHz SPI, 1 MHz CAN-FD> |
| **Memory / Silicon Footprint** | <e.g., 32 KB SRAM, 256 KB ReRAM, ~1.4 mm²> |
| **Target Compliance Standards** | <e.g., AEC-Q100 Grade 0, ISO 26262 ASIL-D, MIL-STD-883K> |

## Subsystem Block Architecture
- **Control Plane**: <CPU core, bus crossbar, interrupt controllers, DMA>
- **Datapath & Accelerators**: <CORDIC, DSP MAC, FFT hardware engine, crypto accelerator>
- **Mixed-Signal & Analog**: <ADCs, DACs, sense amplifiers, PLL, bandgap reference>
- **Power & Gate Drivers**: <Half-bridge gate pre-drivers, buck-boost converter, LDOs>
...
## Deep Engineering Questions & Failure Modes *(Mandatory for Technical Books)*
1. **<Subsystem / Physics Concern>**: <Rigorous trade-off question regarding substrate noise isolation, bus contention, or clock jitter>
2. **<Fault Tolerance & Safety>**: <Question on failure modes, SECDED ECC, lockstep divergence, or fail-safe latching>
3. **<Environmental / Compliance Stress>**: <Question on thermal derating at 150°C, EMP surge clamping, or DO-160G airborne certification>

## Code Examples *(technical books only — omit if BOOK_TYPE=text)*
```<language>
<key code or RTL/verilog snippet from this chapter>
```
...
<!-- Reproduce or reconstruct one concrete example the author works through: a
     sample design-in decision, a filled-in procurement scorecard, a timing waveform analysis,
     or a decision walked end-to-end. -->
...
- **<Standard/PDK>**: <external standard, foundry PDK, or procurement program it connects with>
```
...
1. **Extract structure and physics, not summaries** — capture named frameworks, exact parameters, physical limits, and failure modes; not generic book recaps
2. **Strict Vector Asset Integrity** — NEVER generate synthetic AI images or redraw ugly/primitive boxes. Extract and link authentic 1:1 vector PostScript/PDF pages.
3. **Preserve the author's precision** — "The 5 Whys" ≠ "ask why multiple times"; keep exact naming, pinout designations, and math formulas.
4. **Mandatory Deep Engineering Questions** — for technical chapters, formulate 3-5 rigorous questions covering mixed-signal isolation, bus contention, clocking, thermal limits, and safety standards.
5. **Density over completeness** — a 1,000-token high-density technical audit beats a 10,000-token verbose excerpt.
6. **Practitioner voice** — write "Use X when Y", "Design rule: isolate 120V flyback with deep trench N-well", not "The book explains X".
7. **Front-load SKILL.md** — compaction keeps the first 5,000 tokens; most important content comes first.
8. **Chapter files are on-demand** — they don't count against skill budget until loaded.
9. **Never copy raw book text verbatim** — always synthesize, summarize, extract signal.
10. **Topic index & vector links are critical** — it's how the agent navigates to the right chapter file and authentic asset.
