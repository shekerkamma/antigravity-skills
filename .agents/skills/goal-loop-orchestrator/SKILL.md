...
4. **Use real-data research for strategy.** For moat, market, wedge,
   ...
   lightweight scan. Probe for and use Reddit evidence tools, Exa, Firecrawl,
   durable memory, specialist research skills, primary sources, and
...
- Market wedge with buyer pain: `GBrain/prior recall -> exa-api -> firecrawl -> Reddit semantic chain -> ai-analyst validation -> strategy synthesis`.
...
3. Capability probe: before external research, check which research routes are
   ...
   - MCP/plugin: Exa (`mcp__claude_ai_Exa__web_search_exa` or equivalent),
     Firecrawl (`mcp__firecrawl__*` or `/firecrawl`), Reddit/browser research
     plugins, and host-provided tool discovery.
   - Subagents: Claude `Task`, Codex `multi_agent_v1.spawn_agent`, or another
     available multi-agent tool.
4. Exa: use Exa or an equivalent semantic search tool for source discovery when
   ...
   Exa MCP tool is exposed, use `skills/exa-api/scripts/exa_search.sh` when
   `EXA_API_KEY` is set.
5. Firecrawl: use Firecrawl or an equivalent crawler/reader to ingest important
   ...
   Firecrawl blocked.
6. Reddit evidence: when buyer pain, skepticism, switching triggers, comparison
   ...
   - Then use the best available Reddit retrieval route: supplied thread URLs
     with `reddit-seo-pipeline`, logged-in Reddit/browser research, Exa as a
     Reddit discovery assistant, ScrapeCreators, or
...
If Exa, Firecrawl, Reddit tools, and subagents are all unavailable, either ask
...
7. Produce a handoff.
8. Decide: stop, continue, refine, or ask.
...
| `exa-api` | Research discovery | when current source discovery, competitor discovery, buyer-language page discovery, docs, forums, or market signals are needed | Exa discovery JSON, source candidates, rejected leads |
...
- **Do not validate strategy with simple search.** For moat, wedge, market, and
  ...
  buyer pain or defensibility. Use deeper research routes or label the output as
  a hypothesis.
...
- **Do not skip the capability probe.** For strategy research, explicitly check
  for Reddit, Exa, Firecrawl, and subagent routes before falling back. Missing
  tools are a limitation to report, not a reason to pretend the evidence is
  stronger than it is.
